...
Note: Pillow is no longer required; a pure-Python PNG generator replaces it.
"""
...
class ProviderManager:
    ...
    def _init_providers(self):
        ...
        self.image_providers = [
            ...
            # 优先级 9: 备用 - 纯 Python PNG 生成 (总是可用)
            PurePythonImageProvider("Pure Python Fallback", priority=9),
        ]
...
class PurePythonImageProvider(AIProvider):
    """Provider 9: 纯 Python PNG 生成器 - 总是可用, 无需外部库"""
    
    def _create_png(self, width: int, height: int, prompt: str) -> bytes:
        """生成一个简单的 PNG 图像 (渐变 + 随机噪声 + 提示词元数据)"""
        # PNG 签名
        png_signature = b'\x89PNG\r\n\x1a\n'
        
        # IHDR 块: 宽度, 高度, 位深, 颜色类型, 压缩, 滤波, 交错
        ...
        # 创建图像数据: 每个像素 3 字节 (RGB)
        data = bytearray()
        for y in range(height):
            # 滤波: 第0字节为 None (0)
            data.append(0)
            for x in range(width):
                # 简单的渐变: 红分量随 x 变化, 绿随 y, 蓝为混合
                r = (x * 255) // width
                g = (y * 255) // height
    ...
    async def generate_image(self, prompt: str) -> Optional[bytes]:
        try:
            # 生成一个 1024x1024 的 PNG
            return self._create_png(1024, 1024, prompt)
        except Exception as e:
            logger.error(f"PurePythonImage error: {e}")
            return None
